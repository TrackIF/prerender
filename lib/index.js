...
exports = module.exports = function(options) {
    ...
    if(cluster.isMaster) {
        ...
        cluster.on('online', function (worker) {
            util.log('worker #' + worker.id + ' :: started!');
        });

        cluster.on('listening', function(worker, address) {
          util.log("A worker is now connected to " + address.address + ":" + address.port);
        });

        cluster.on('disconnect', function(worker) {
            util.log('worker #' + worker.id + ' :: disconnected');
        });

        cluster.on('exit', function (worker) {
            if (worker.suicide === true) {
                util.log('worker #' + worker.id + ' :: died, it was just suicide – no need to worry');
            } else {
